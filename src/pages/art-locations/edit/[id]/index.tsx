...
import { getArtLocationById, updateArtLocationById } from 'apiSdk/art-locations';
...
function ArtLocationEditPage() {
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<ArtLocationInterface>(
    () => (id ? `/art-locations/${id}` : null),
    () => getArtLocationById(id),
  );
  ...
  const handleSubmit = async (values: ArtLocationInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateArtLocationById(id, values);
      mutate(updated);
      resetForm();
